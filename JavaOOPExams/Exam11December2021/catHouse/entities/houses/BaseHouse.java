...
public abstract class BaseHouse implements House {
    ...
    public String getStatistics() {
        String catsOutput = cats.size() == 0 ? "none" : cats.stream().map(Cat::getName).collect(Collectors.joining(" "));

        return String.format("%s %s:%n" +
                "Cats: %s%n" +
                "Toys: %d Softness: %d", name, getClass().getSimpleName(), catsOutput, toys.size(), sumSoftness());
    }
